...
namespace HaikyuuGame.Editor
{
    public static class ProjectValidator
    {
        ...
        public static void ValidateProjectData()
        {
            ...
            if (roster.Count != 48) throw new InvalidOperationException($"Expected 48 roster entries, found {roster.Count}.");
            ...
            for (int i = 0; i < roster.Count; i++)
            {
                ...
                if (string.IsNullOrWhiteSpace(profile.Id) || !ids.Add(profile.Id)) throw new InvalidOperationException($"Invalid or duplicate character id at index {i}: {profile.Id}");
                if (string.IsNullOrWhiteSpace(profile.DisplayName)) throw new InvalidOperationException($"Character {profile.Id} is missing a display name.");
            ...
            for (int i = 0; i < PresetIds.Length; i++)
            {
                ...
                if (preset == null || preset.StartingSix == null || preset.StartingSix.Length != 6) throw new InvalidOperationException($"Invalid team preset: {PresetIds[i]}");
            }

            if (StoryCampaignCatalog.All.Count != 10) throw new InvalidOperationException($"Expected 10 story chapters, found {StoryCampaignCatalog.All.Count}.");
            Debug.Log($"Project validation passed: {roster.Count} playable characters, {schools.Count} schools/groups, {PresetIds.Length} team presets, {StoryCampaignCatalog.All.Count} story chapters.");
        }

        private static void ValidateStat(string id, string stat, int value)
        {
            if (value < 1 || value > 100) throw new InvalidOperationException($"{id}.{stat} must be 1..100, got {value}.");
        }
    }
}
#endif
