...
namespace HaikyuuGame.Meta
{
    public sealed class GameShellController : MonoBehaviour
    {
        ...
        private void SelectMode(GameMode mode)
        {
            ...
            _tuning.quickMatch = mode == GameMode.QuickMatch || mode == GameMode.Training || mode == GameMode.Challenge || mode == GameMode.DreamTeam;
            if (mode == GameMode.Tournament) _tournamentPairings = TournamentBracket.Generate(TournamentTeams, System.Environment.TickCount);
            _menuOpen = false;
            Time.timeScale = 1f;
        ...
        private void OnGUI()
        {
            ...
            if (Time.unscaledTime < _toastUntil) GUI.Box(new Rect((Screen.width - 400f) * 0.5f, 205f, 400f, 42f), _toast);
        }
        ...
        private void DrawModeMenu()
        {
            ...
            if (_tournamentPairings != null && _tournamentPairings.Count > 0)
            {
                ...
                GUI.Label(new Rect(x + 28f, y + 372f, width - 56f, 24f), $"Tournament seed: {pair.Left} vs {pair.Right} | Round {GameSessionState.TournamentRound + 1}");
            }
