...
namespace HaikyuuGame
{
    public sealed class PlayableCoreBootstrap : MonoBehaviour
    {
        ...
        {
            if (FindFirstObjectByType<PlayableCoreBootstrap>() != null) return;
        ...
        private void Start()
        {
            ...
            PerfectContactFeedback perfectFeedback = gameObject.AddComponent<PerfectContactFeedback>();
            perfectFeedback.Initialize(ball);
        ...
        private void BuildCourt()
        {
            ...
            CreateLine(new Vector3(_tuning.halfCourtLength, 0.015f, 0f), new Vector3(0.055f, 0.025f, _tuning.halfCourtWidth * 2f));
            CreateLine(new Vector3(-3f, 0.018f, 0f), new Vector3(0.045f, 0.026f, _tuning.halfCourtWidth * 2f));
            CreateLine(new Vector3(3f, 0.018f, 0f), new Vector3(0.045f, 0.026f, _tuning.halfCourtWidth * 2f));
        }
        ...
        private void BuildTeams(VolleyballBall ball, out TeamRotation leftRotation, out TeamRotation rightRotation)
        {
            Vector3[] leftFormation = CreateFormation(TeamSide.Left);
            Vector3[] rightFormation = CreateFormation(TeamSide.Right);
            string[] leftIds = { "kageyama_tobio", "asahi_azumane", "hinata_shoyo", "daichi_sawamura", "tanaka_ryunosuke", "tsukishima_kei" };
            string[] rightIds = { "miya_atsumu", "aran_ojiro", "suna_rintaro", "miya_osamu", "kita_shinsuke", "omimi_ren" };
            List<PlayerActor> leftRoster = new List<PlayerActor>();
            List<PlayerActor> rightRoster = new List<PlayerActor>();
            for (int i = 0; i < 6; i++)
            {
                leftRoster.Add(CreatePlayer(TeamSide.Left, i == 2, HaikyuuRosterCatalog.Get(leftIds[i]), leftFormation[i], ball));
                rightRoster.Add(CreatePlayer(TeamSide.Right, false, HaikyuuRosterCatalog.Get(rightIds[i]), rightFormation[i], ball));
            }
            PlayerActor leftLibero = CreatePlayer(TeamSide.Left, false, HaikyuuRosterCatalog.Get("nishinoya_yu"), new Vector3(-12f, 1f, 0f), ball);
        ...
        private static Vector3[] CreateFormation(TeamSide team)
        {
            ...
            return new[] { new Vector3(sign * 2.1f, 1f, -2.5f), new Vector3(sign * 2.1f, 1f, 0f), new Vector3(sign * 2.1f, 1f, 2.5f), new Vector3(sign * 6f, 1f, 2.5f), new Vector3(sign * 6f, 1f, 0f), new Vector3(sign * 6f, 1f, -2.5f) };
        }

        private PlayerActor CreatePlayer(TeamSide team, bool human, RuntimeCharacterProfile profile, Vector3 position, VolleyballBall ball)
        {
            ...
            if (collider != null) collider.enabled = false;
            Color teamColor = team == TeamSide.Left ? new Color(0.12f, 0.12f, 0.16f) : new Color(0.12f, 0.58f, 0.72f);
            if (profile != null && profile.Role == VolleyballRole.Libero) teamColor = team == TeamSide.Left ? new Color(0.12f, 0.55f, 0.25f) : new Color(0.78f, 0.24f, 0.22f);
            if (human) teamColor = new Color(1f, 0.52f, 0.05f);
